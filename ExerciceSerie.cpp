...
ExerciceSerie::ExerciceSerie(QMainWindow *p)
{
    ...
    for(int i=0;i<10;i++){
        calcul[i+10] = new  QLabel(QString::number(i+11) +" x "+ QString::number(taVariable) +" =",this);
        calcul[i+10]->setGeometry(230+100, 10*i+15+40+30*i+80,90,40);
        calcul[i+10]->setFont(QFont("PncilPete FONT", 14));
    }
    for(int i=0;i<10;i++){
        calcul[i+20] = new  QLabel(QString::number(i+21) +" x "+ QString::number(taVariable) +" =",this);
